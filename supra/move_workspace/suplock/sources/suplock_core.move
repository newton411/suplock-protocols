...
module suplock::suplock_core {
    ...
    const ERR_YIELD_ALREADY_CLAIMED: u64 = 1012;
    ...
    public fun calculate_yield(
        ...
        let boost = calculate_boost_multiplier(lock_duration_secs);
        let base_yield = ((amount as u128) * (global_state.base_apr_bps as u128) * (lock_duration_secs as u128))
            / 10000 / 31_536_000;
        let total_yield = (base_yield * boost) / 10000;
        (total_yield as u64)
    }
    ...
    public fun claim_yield(
        ...
        assert!(!lock.yield_claimed, ERR_YIELD_ALREADY_CLAIMED); // Yield already claimed
